...
var allDomainsRange, domains, lastSearches, listURL, logSearch, search, searchboxChanged, updateList, verifyMatchData;
...
String.prototype.matchAllWithIndexes = function(match) {
  var exact, i, indexes, j, k, ref, ref1, start;
  if (!verifyMatchData(this, match)) {
    return {
      matches: false
    };
  }
  start = this.indexOf(match[0]);
  if (start === -1) {
    return {
      matches: false
    };
  ...
  indexes = [start];
  j = 1;
  for (i = k = ref = start, ref1 = this.length; ref <= ref1 ? k < ref1 : k > ref1; i = ref <= ref1 ? ++k : --k) {
    if (this[i] === match[j]) {
      indexes.push(i);
      if (++j === match.length) {
        return {
